  ...
  return (
    ...
    <div className={`flex py-1 mb-2 flex-col md:flex-row md:justify-start shadow-xl justify-center items-center sticky top-0 z-10 bg-white ${sidebar&&'overflow-hidden'}`}>
      <Link href={"/"}>
          ...
          <div className="logo mr-16 flex items-center justify-start text-gray-900 ml-4"> 
            <Image src={'/logo.png'} alt="The Crafted Treasures" width={80} height="80" />
            <span className='text-3xl md:text-3xl font-serif text-pink-800 ml-4'><b>The Crafted Treasures</b></span> 
          </div>
      ...
      <div classNameEmbroidery="nav">
        <ul className='flex items-center space-x-4 mx-4 font-bold'>
          ...
          <Link href={"/embroidery"}><a><li className={'hover:text-blue-600'}>Embroidery</li></a></Link>
        </ul>
      </div>
      ...
      <div ref={ref} className={`w-72 h-[100vh] overflow-y-auto sideCart absolute top-0 p-5 bg-blue-200 px-8 transition-all ${sidebar? '-right-80': 'right-0'}`}>
        ...
        <ol className='list-decimal font-semibold px-4'>
          ...
          {cart && Object.keys(cart).length > 0 && Object.keys(cart).map((k) => {
            return <li key={k}>
        ...
        <div className="flex">
          <Link href={"/checkout"}>
              ...
              <button disabled={Object.keys(cart).length === 0} className="disabled:bg-pink-300 flex mx-1 mt-16 text-white bg-pink-500 border-0 py-2 px-2 focus:outline-none hover:bg-pink-600 rounded text-lg"> <BsFillBagCheckFill className='mt-1 mx-1' />  checkout </button>
